...
**Repo:** `xingai-enterprise-ai-pocs/pocs/claims-workflow-v2-poc` · **Status (2026-07-16):** Runnable · Phase 1+2+3 · 40 tests passing
...
This is the single most cross-referenced product in this wiki — nearly every course connects to some piece of it. See each course page's "Connects to" section rather than duplicating the mapping here.
...
## PRODUCTION-READINESS.md — the gap analysis this wiki treats as a Course 06 case study
...
- [Concept: Decision Ledger pattern](../concepts/decision-ledger-pattern.md), [Concept: Cache-first LLM architecture](../concepts/cache-first-llm-architecture.md) (the heuristic/LLM dual-path is a fallback discipline, not a cache, but shares the "never let one call be a single point of failure" instinct), [Concept: Agent governance and MCP](../concepts/agent-governance-and-mcp.md)
- [Course 05](../courses/05-agent-runtime-multi-agent.md) (LangGraph, state machines), [Course 06](../courses/06-production-ai-engineering.md) (this POC's own production-readiness gap analysis), [Course 07](../courses/07-enterprise-decision-systems.md) (Decision Ledger), [Course 04](../courses/04-mcp-interoperability.md) (the MCP server itself)
- `claims-mcp-oauth-poc` — the real OAuth 2.1 Authorization Server this POC's ADR-009 Phase 4 plans to wire in front of `mcp_server/`; not yet ingested into this wiki.
- [xingai-agent-firewall](xingai-agent-firewall.md) — a sibling POC's prompt-injection gap (this page) and Firewall's runtime tool-call governance are the same underlying risk class, addressed in two different products, neither addressing the other's half yet.
